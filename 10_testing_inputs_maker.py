...
#function to convert height,weight,type into 6-bit and 3-bit sequences
def encoder_function(height, weight, kind):
    ...
    #encode unique 3-bits for every 1.5kg
    if weight <= 17.0:
        wt_bits = "000"
...
#defining the testing files here
input_file = "9_testing_data_table.txt"
output_file_1 = "11_testing_inputs.mem"
output_file_2 = "11_testing_exp_outputs.mem"
...
with open(input_file, "r") as file: #opening the table text file in read mode
    ...
    for line in lines[2:]:  #skipping lines like height,weight,age,kind (refer to the file to see it)
        parts = line.strip().split("|") #extract the numerical values from the table
        if len(parts) >= 5: #extract height, weight, age, kind seperately
            try:
                ...
                sequences.append(result[:6]) #add height,weight,age to the list of sequences
